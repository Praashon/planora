...
router.post("/api/projects/save", async ({ request, user }) => {
  try {
    ...
    const payload = {Write a handleExport function that downloads the currently rendered image in the browser in app/routes/visualizer.$id.tsx. If currentImage is falsy, return early.
      ...project,
...
router.get("/api/projects/list", async ({ user }) => {
  try {
    ...
    return jsonError(500, "Failed to get projects", {
      message: e.message || "Unknown error",
    });
  }
});
...
router.get("/api/projects/get", async ({ request, user }) => {
  try {
    ...
    if (!id) return jsonError(400, "Project ID not found");
